...
import { defaultRules, applyRuleMutation, validateRules, type UserRules } from '@/domain/rules';
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-4xl gap-6 p-6">
            ...
            <li key={t.id}>
              <button
                ...
                className={`w-full rounded px-3 py-1.5 text-left text-sm ${
                  tab === t.id ? 'bg-slate-800 text-white' : 'hover:bg-black/5'
                }`}
              >
                {t.label}
              </button>
      ...
      <main className="min-w-0 flex-1">
          ...
          <>
            <Section title="Mode">
              ...
              <p className="mt-2 text-xs opacity-70">
                Strict mode hides moderate-confidence content and will mislabel some human-made
                videos.
              </p>
            </Section>
                  ...
                  <tr key={category}>
                    ...
                    <td>
                      <select
                        ...
                        className="block w-40 cursor-pointer rounded-md border border-black/20 bg-white px-2 py-1.5 text-sm shadow-sm focus-visible:border-slate-500 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-slate-600"
                      >
        ...
        {(tab === 'allowed' || tab === 'blocked') && (
          <RuleListSection tab={tab} rules={rules} onChange={setRules} />
        )}
        ...
        {tab === 'privacy' && (
          <>
            <Section title="YouTube feedback">
              ...
              />
              {settings.youtubeFeedback.enabled && (
                <p role="status" className="mt-2 rounded bg-amber-50 p-2 text-xs">
                  Currently not wired: this build performs no YouTube actions. Your preference is
                  stored for a future release.
                </p>
              )}
            </Section>
            <Section title="Community reputation">
              ...
              />
              {settings.remoteProvider.enabled && (
                <p role="status" className="mt-2 rounded bg-amber-50 p-2 text-xs">
                  Currently not wired: no remote provider is used in this build. Your preference is
                  stored for a future release.
                </p>
              )}
            </Section>
...
function RuleListSection({
  ...
  const removeChannel = (channelId: string) => {
    onChange(
      isAllowed
        ? { ...rules, allowedChannelIds: rules.allowedChannelIds.filter((c) => c !== channelId) }
        : { ...rules, blockedChannelIds: rules.blockedChannelIds.filter((c) => c !== channelId) },
    );
  };
    ...
    <Section title={isAllowed ? 'Explicitly allowed' : 'Explicitly blocked'}>
      ...
      {videoIds.length > 0 && (
        <>
          ...
          <ul className="text-sm">
            {videoIds.map((id) => (
              <li key={id} className="flex items-center justify-between py-0.5">
        ...
        <>
          ...
          <ul className="text-sm">
            {channelIds.map((id) => (
              <li key={id} className="flex items-center justify-between py-0.5">
                <code className="text-xs">{id}</code>
                <Button onClick={() => removeChannel(id)}>Remove</Button>
              </li>
            ))}
          </ul>
        </>
      ...
      {handles.length > 0 && (
        <>
          ...
          <ul className="text-sm">
            {handles.map((h) => (
              <li key={h} className="flex items-center justify-between py-0.5">
                <code className="text-xs">@{h}</code>
                <Button onClick={() => removeHandle(h)}>Remove</Button>
              </li>
            ))}
          </ul>
        </>
      ...
      {!isAllowed && (
        <PhraseEditor
          ...
          onAdd={(phrase) => onChange(applyRuleMutation(rules, { kind: 'block-phrase', phrase }))}
          onRemove={(phrase) =>
...
/** CFG-05: literal phrase block rules — create, remove, live preview note. */
function PhraseEditor({
  ...
  return (
    <div className="mt-3">
      ...
      <p className="mb-2 text-xs opacity-70">
        Hide any video whose title contains one of these exact phrases (case-insensitive, literal
        text — no wildcards). Checked before your Not-AI corrections.
      </p>
      <form
      ...
      {phrases.length > 0 && (
        <ul className="mt-2 text-sm">
          {phrases.map((p) => (
            <li key={p} className="flex items-center justify-between py-0.5">
              <span>“{p}”</span>
              <Button onClick={() => onRemove(p)}>Remove</Button>
            </li>
          ))}
        </ul>
      )}
...
function ReviewSection({
  ...
  const applyRule = async (mutation: Parameters<typeof applyRuleMutation>[1]): Promise<void> => {
    const raw = await browser.storage.local.get('local:rules');
    const current = validateRules(raw['local:rules']) ?? defaultRules();
    await browser.storage.local.set({ 'local:rules': applyRuleMutation(current, mutation) });
  };
      ...
      <Section title="Review history">
        ...
        {!loading && error === null && result !== null && result.total > 0 && (
          <>
            ...
            {selection.size > 0 && (
              <div className="mb-2 flex flex-wrap items-center gap-2 rounded bg-black/5 p-2 text-sm">
            ...
            {bulkNotice !== null && (
              <div className="mb-2 rounded bg-black/5 p-2 text-sm" role="status">
            ...
            <ul className="space-y-3">
              {items.map((summary) => {
                ...
                return (
                  <li key={summary.key} className="rounded border border-black/10 p-3 text-sm">
        ...
        <Section title="Not yet migrated (old history)">
          ...
          <ul className="space-y-2">
            {records.slice(0, 10).map((record) => (
              <li key={record.id} className="rounded border border-black/10 p-2 text-sm">
